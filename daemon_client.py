...
class DaemonClient:
    ...
    def get_current_joint_positions(self) -> Tuple[list[float], list[float]]:
        ...
        Returns:
            ...
            - head_joints: [body_yaw, stewart_1, stewart_2, ..., stewart_6] in radians
            - antenna_joints: [left_antenna, right_antenna] in radians

        Note: This matches ReachyMini SDK format for compatibility.
        Note: Daemon API changed - no longer exposes raw joint positions.
              We return body_yaw + zeros for stewart platform (not used).
        """
        try:
            # Get body yaw
            ...
            body_yaw_response.raise_for_status()
            ...
            antenna_response.raise_for_status()
            antenna_joints = antenna_response.json()  # Returns [left, right]

            # Reconstruct head_joints format: [body_yaw, stewart_1, ..., stewart_6]
            # Stewart platform joint positions not exposed by new API, use zeros
            head_joints = [body_yaw] + [0.0] * 6

            return (head_joints, antenna_joints)
