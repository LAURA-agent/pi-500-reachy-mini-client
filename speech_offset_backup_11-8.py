...
class SpeechOffsetPlayer:
    ...
    def _generate_timeline(self, analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        pitch_events = self._generate_pitch_events(audio_feat, mood_scale)
        events.extend(pitch_events)

        # 2. Generate energy peak → head nod events (discrete, preserving Z and antennas from pitch)
        events.extend(self._generate_peak_events(audio_feat, mood_scale, pitch_events))
    ...
    def _generate_pitch_events(self, audio_feat: Dict, mood_scale: float) -> List[Dict]:
        ...
        for t, pitch, conf in zip(times, values, confidence):
            ...
            events.append({
                'time': float(t),
                'offsets': (0.0, 0.0, head_z, 0.0, 0.0, 0.0),  # Small Z offset
    ...
    def _generate_peak_events(self, audio_feat: Dict, mood_scale: float, pitch_events: List[Dict]) -> List[Dict]:
