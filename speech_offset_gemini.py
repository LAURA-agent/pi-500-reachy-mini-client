...
### **2. Corrected `speech_offset.py`**

This version introduces the **-10° baseline pitch** and makes the head nods **additive** to that baseline, creating much more expressive and noticeable movement.
...
class SpeechOffsetPlayer:
    ...
    def _generate_timeline(self, analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        baseline_pitch_rad = np.deg2rad(-10.0)  # Default -10° pitch during speech
    ...
    def _generate_pitch_events(self, audio_feat: Dict, mood_scale: float, baseline_pitch_rad: float) -> List[Dict]:
        ...
        for t, pitch, conf in zip(times, values, confidence):
            ...
            events.append({
                'time': float(t),
                'offsets': (0.0, 0.0, head_z, 0.0, baseline_pitch_rad, 0.0),  # Z offset + baseline pitch
    ...
    def _generate_peak_events(self, audio_feat: Dict, mood_scale: float, pitch_events: List[Dict], baseline_pitch_rad: float) -> List[Dict]:
        ...
        for peak_time in peak_times:
            ...
            events.append({
                ...
                'offsets': (0.0, 0.0, z_value, 0.0, nod_amplitude + baseline_pitch_rad, 0.0),
            ...
            events.append({
                'time': float(end_time),
                'offsets': (0.0, 0.0, z_value_end, 0.0, baseline_pitch_rad, 0.0),
    ...
    def _generate_pause_events(self, audio_feat: Dict, baseline_pitch_rad: float) -> List[Dict]:
        ...
        for pause in pauses:
            ...
            events.append({
                ...
                'offsets': (0.0, 0.0, 0.0, 0.0, baseline_pitch_rad, 0.0),
                'antennas': (0.0, 0.0)
            })
    ...
    def _generate_question_events(self, duration: float, baseline_pitch_rad: float) -> List[Dict]:
        ...
        final_pitch = tilt_amplitude + baseline_pitch_rad

        events = [
            {'time': tilt_start, 'offsets': (0.0, 0.0, 0.0, 0.0, final_pitch, 0.0), 'antennas': (0.0, 0.0)},
            {'time': duration, 'offsets': (0.0, 0.0, 0.0, 0.0, final_pitch, 0.0), 'antennas': (0.0, 0.0)},
        ]
