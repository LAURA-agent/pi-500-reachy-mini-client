...
def get_move_duration(emotion_name):
    """Get the duration of a move from local HuggingFace cache.

    Returns:
        float: Duration in seconds, or 4.0 as fallback
    """
    # Path to locally cached emotion moves
    cache_path = Path("/home/user/.cache/huggingface/hub/datasets--pollen-robotics--reachy-mini-emotions-library/snapshots/152e84b8f46b88c4b52dd34bbef6975637366177")
    move_file = cache_path / f"{emotion_name}.json"

    try:
        if not move_file.exists():
            log_debug(f"Move file not found: {move_file}, using 4.0s fallback")
            return 4.0

        with open(move_file, 'r') as f:
            move_data = json.load(f)

        # Duration is the maximum time value in the trajectory
        if 'time' in move_data and len(move_data['time']) > 0:
            duration = max(move_data['time'])
            log_debug(f"Move '{emotion_name}' duration: {duration:.2f}s")
            return duration
        else:
            log_debug(f"No time data in move file, using 4.0s fallback")
            return 4.0
    ...
    while time.time() - start_time < timeout:
        if tts_output_dir.exists():
            ...
            if mp3_files:
                ...
                # Check if it's new (created within last 30 seconds)
                # Increased to 30s to handle API-triggered moods where TTS played before trigger
...
def play_mood_loop_debug(mood_name, max_duration=60):
    ...
    Algorithm:
    1. Play first move IMMEDIATELY (no TTS wait)
    2. After first move, check for TTS file (2s timeout)
    3. If TTS found and first move < TTS duration: continue with remaining moves
    4. Otherwise: exit and resume breathing
    """
    log_debug(f"=" * 60)
    ...
    try:
        # PHASE 1: Play first move IMMEDIATELY (no TTS wait)
        log_debug("\n=== PHASE 1: Playing first move immediately ===")

        first_emotion = random.choice(emotions)
        first_duration = get_move_duration(first_emotion)
        log_debug(f"First move: {first_emotion} ({first_duration:.2f}s)")

        success, move_uuid, actual_duration = play_emotion(first_emotion)
        if not success:
            log_debug("Failed to play first move, exiting")
            return

        moves_played = 1
        first_move_time = actual_duration + buffer_per_move
        log_debug(f"First move playing, waiting {first_move_time:.2f}s...")
        time.sleep(first_move_time)

        # PHASE 2: Check for TTS file after first move
        log_debug("\n=== PHASE 2: Checking for TTS file ===")
        tts_file, tts_duration = wait_for_tts_file(timeout=2.0)

        if tts_file is None or tts_duration == 0:
            log_debug("No TTS file found after 2s timeout, exiting")
            return

        log_debug(f"TTS duration: {tts_duration:.2f}s")
        log_debug(f"First move used: {first_move_time:.2f}s")

        remaining_duration = tts_duration - first_move_time
        log_debug(f"Remaining TTS time: {remaining_duration:.2f}s")

        if remaining_duration <= 0:
            log_debug("First move already exceeded TTS duration, exiting")
            return

        # PHASE 3: Fill remaining TTS duration with moves
        log_debug("\n=== PHASE 3: Playing remaining moves ===")

        while remaining_duration > 0:
            # Filter emotions to only those that fit
            fitting_emotions = [e for e in emotions if get_move_duration(e) + buffer_per_move <= remaining_duration]

            if not fitting_emotions:
                log_debug(f"\n--- No moves fit in remaining {remaining_duration:.2f}s, exiting ---")
                break

            emotion = random.choice(fitting_emotions)
            move_duration = get_move_duration(emotion)
            log_debug(f"\n--- Move {moves_played + 1}: {emotion} ({move_duration:.2f}s) fits in remaining {remaining_duration:.2f}s ---")
        ...
        # Wait for daemon queue to empty
        log_debug("\n--- Waiting for daemon moves to complete ---")
        max_wait = 3.0
        wait_start = time.time()
        while time.time() - wait_start < max_wait:
            running_moves = get_running_moves()
            if len(running_moves) == 0:
                log_debug("Daemon queue is empty, ready to resume")
                break
            log_debug(f"Waiting for {len(running_moves)} moves to complete...")
            time.sleep(0.2)
